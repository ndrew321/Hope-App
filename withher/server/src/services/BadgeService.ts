...
export const BadgeService = {
   ...
   */
  async checkAfterSessionCompleted(userId: string): Promise<void> {
   ...
   */
  async checkAfterPostCreated(userId: string): Promise<void> {
   ...
   */
  async checkAfterProfileUpdated(userId: string): Promise<void> {
   ...
   */
  async checkAfterEventAttended(userId: string): Promise<void> {
   ...
   */
  async checkAfterLogin(userId: string): Promise<void> {
    ...
    for (let i = 0; i < sortedDays.length; i++) {
      if (sortedDays[i] === today.getTime() - i * 86400000) streak++;
      else break;
    }

    if (streak >= 7) await _awardBadge(userId, BADGE_TYPES.STREAK_7);
    if (streak >= 30) await _awardBadge(userId, BADGE_TYPES.STREAK_30);
  },
};
 ...
 */
async function _awardBadge(userId: string, badgeType: string): Promise<void> {
  const badge = await prisma.badge.findUnique({ where: { badgeType } });
  if (!badge) {
    logger.warn({ badgeType }, 'Badge type not found in database');
    return;
  }
  ...
  await NotificationService.sendBadgeEarnedNotification(userId, badge.name);
  logger.info({ userId, badgeType }, 'Badge awarded');
}
